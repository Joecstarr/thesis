...
trio ultimately constructed a complete list of prime knots up to ten crossings
(250+1 knots). When the knot theoretic machinery available at the time is
considered, the completion of these tables with such high accuracy was a
Herculean task. The tables contained a single error, two equivalent ten crossing
knots (@fig-hist-perko), identified in 1974 by Perko an amateur mathematician
[@perkoClassificationKnots1974].

```{figure} ../../media/perko_pair.svg
:label: fig-hist-perko
The Perko pair, a pair of ten equivalent ten crossing knots appearing as an
...
only four omissions[@caudron1982classification]. Conway continued to work by
hand computation, but employed a novel approach to tabulation. He described
decompositions of knots into building blocks, which he called
tangles[@conwayEnumerationKnotsLinks1970], and a calculus to glue the blocks
together. Under Conway's tangle calculus, the combinatorial work of knot
tabulation became a game of building from simple to complex. Inspired by
...
became closer to commodity products, allowing for researchers with diverse
methods to take their crack at time-consuming computational tasks. One of these
computational tasks was the construction of knot tables. The first to construct
a knot table by computer was Dowker and Thistlethwaite in 1983
[@dowkerClassificationKnotProjections1983], who produced a table of all prime
knots to thirteen crossing. The pair implemented a novel two pass approach that
has served as an outline for all major efforts that have followed. The process
begins with a first pass to enumerate all possible knot diagrams. This is
followed by a second pass where "sufficient invariants to distinguish them
(knots) from each other" [@dowkerClassificationKnotProjections1983] are
computed. This effectively assigns knots to equivalence classes (bins [^bins])
hence finding and removing all duplicate entries from the list (deduping
[^dedupe]). The next table produced by computer, knots up to sixteen crossing,
was given by Hoste, Thistlethwaite, and Weeks [@hosteFirst1701936Knots1998]
in 1998. Their process deviates only slightly from the earlier approach of
Dowker and Thistlethwaite, leaning on a heuristics[^heuristics] to limit the
duplicates found in their first pass. This preprocessing in the first pass
allowed Hoste, Thistlethwaite, and Weeks to compute their table in one to two
weeks of wall time[^wall-time]. The most recent computational effort was carried
out by Burton in 2020 [@burtonNext350Million2020], finding knots to nineteen
...
serving as an important signpost for the time requirement problems of knot
tabulation.
...
[^heuristics]:
    When discussing algorithms, a heuristic often describes a special case that
    when seen short circuits the algorithm, reducing unnecessary work.
