...
Before we can develop the concept of writhe of a knot, we need to define an
orientation for a knot. Imagine the strands of a knot to be a roller coaster,
it's clear that the roller coster car has two choices for direction. One
direction we intuitively call forward and its oposite backwards. Just like a
rollar coaster, each strand of a knot is a circle we can choose to traverse in
one of two directions. On a knot we call a choice of forward, an **orientation**
...
We now zoom in on our oriented diagram, and focus on a crossing. Since the whole
...
by rotating, so that the over strand arrowhead points north, notice then that
...
crossing (@fig-inv-or-positive), while an east pointing under strand is called a
negative (-) crossing (@fig-inv-or-negative).
...
Given an oriented knot, we can now classify each crossing as either a positive
or negative, providing us with enough information to define the writhe of a
knot.
...
An oriented five crossing knot, each local picure around a vertex is magnified
