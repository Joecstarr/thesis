...
the tools he developed and used to achieve those "a few hours" of amazing
efficiency.
...
These boundaries that split knots at four points are called **Conway circles**,
and we call the points $NW,\ NE,\ SW,\ \text{and }SE$ **boundary points**.
Formally, we can consider a Conway circle to be a Jordan curve[^Jordan-curve]
...
isotopy of one into the other.

We move our attention to the three dimensional analog for a Conway circle, the
**Conway sphere**. A Conway sphere, similar to the Conway circle, is an $S^2$
...
that can be created with the construction. For the case of drawing Conway
circles to build tangles, the simplest tangles are the tangle with no crossings
(the $0$ tangle @prelim-fig-basic_0) and the tangle with a single crossing (the
$+1$ tangle @prelim-fig-basic_1).
...
A table with all possible rotations for a generic tangle.
```
...
:label: fig-opo-minus
Rotating a tangle around the $NW$ $SE$ diagonal, yeilding the negative of the
tangle.
```
...
$A\ B$ by first aligning $A$ and $B$ horizontally. We then take $-A$ and sum the
two resulting tangles, equivalent to $-A+B$ (@fig-opo-times).
...
$T$ and rotate (according to the sign of the crossing) around the horizontal
axis of $T$ (@fig-opo-flype).
...
Since Conway's interest was in knots, he natrually needed ways to close up a
tangle to form a knot. In this section, we will introduce two ways that this can
be accomplished. The first closure method is the simple tangle closures, where
...
The first closure methos is the simple tangle closures. For a generic tangle
$T$, we have two options for how to simply close up the tangle. One option is to
...
the calculus, every operation forms a bigon[^bigon] in the knot shadow.
Systematically removing bigons in the knot shadow, deleting edges and merging
vertices, for a knot formed by only these operations we obtain a four-valent
graph[^four-val] with one vertex (@fig-bigon_collapse). The class of knots who
have a presentation where bigons can be collapsed to a single vertex are called
the **algebraic knots**.

```{figure} ../../media/fig-bigon_collapse.svg
:label: fig-bigon_collapse
From left to right: • A trefoil knot. • A knot shadow for a trefoil knot. • A
bigon is highlighted. • The previously highlighed bigon is collapsed and a new
bigon is highlighted. • A graph with no bigons.
```

To obtain a knot that has non-bigon connections between input, we will first
...
or octahedron can be seen in @fig-6starstar.

```{figure} ../../media/fig-6starstar.svg
:label: fig-6starstar
A four valent planar graph with six vertices. When the graph is placed on the
surface of an $S^2$ we get the octahedron.
```
...
tangle insertion into $6^{**}$ can be seen in @fig-6starstar_insurtion.
...
When each vertex has a tangle inserted, the result is a knot. When any vertex is
left empty, the result is a tangle with $n$ boundary Conway spheres, if $n$ is
$1$ we have a tangle in the sense we've been discussing. To reduce ambiguity, in
the graph (orthogonal projection) we mark this sphere with an $\ast$. Following
the termonology outlined by Conolly
[@connollyClassificationTabulation2string2021], we call these singally marked
polygonal graph a **constelation**. We call the tangles generated by this
approach the **polygonal tangles**. For purposes of insertion, on each polygon
graph, we select a canonical ordering of vertices. When noting the tangle
insertions, we list the subtangles we wish to insert in graph cannonical order,
each separated by a period.

```{note}
Observe that insertion into the one vertex graph is equivlent to the simple
tangle closures.
```

[^Jordan-curve]:
    A Jordan curve is a simple closed curve. This can be thought of as a curve
    drawm on a piece of paper that has: 1) No end points. 2) no self
    intersections.

[^bigon]:
    A bigon is a polygon with two sides. In the same way, an octogon has eight
    sides or a trigon (triangle) has three.
