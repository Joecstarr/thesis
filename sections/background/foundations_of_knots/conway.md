...
In this process, we consider each vertex a boundary of a Conway sphere. After
generating a tangle with the Conway calculus, we then insert the tangle into the
Conway sphere (vertex in the graph), connecting the
$NW,\,NE,\,SW,\,\text{and }SE $ points of the tangle to marked points of the
Conway sphere (the four edges of the vertex). An example of a tangle insertion
into $ 6^{**}$
can be seen in @fig-6starstar_insurtion. When each vertex has a tangle inserted,
the result is a knot. When any vertex is left empty, the result is a tangle with
$n$ boundary Conway spheres, if $n$ is $1$ we have a tangle in the sense we've
been discussing. To reduce ambiguity, in the graph (orthogonal projection) we
mark this sphere with an $\ast$. Following the termonology outlined by Conolly
[@connollyClassificationTabulation2string2021] we call these singally marked
polygonal graph a **constelation**.
...
we list the subtangles we wish to insert, each separated by a period. We call
the tangles generated by this approach the **polygonal tangles**.
