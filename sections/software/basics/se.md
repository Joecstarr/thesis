...
product management section (@sec-product-management), we are approaching the software engineering
...
gates[^gate] to be satisfied before moving between those steps. We call this collection of steps and
...
[@beckExtremeProgrammingExplained2012] (@fig-extreme-model) and scrum [@nonakaNewNewProduct1986]
(@fig-scrum-model). Less common in industry but historically relevant are linear models such as the
...
parallelized as in @se-fig-modifiedmultiv. Allowing the process to be utilized by individual
researchers at primarily undergraduate institutions or large REU[^reu] projects.
...
are simplifications of standard UML [@UnifiedModelingLanguage2017] diagrams. Throughout the phases,
...
[^gate]: A collection of quality goals that need to be satisfied to call a step "complete".
