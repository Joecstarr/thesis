...
second included diagrammatically in a class diagram [@UnifiedModelingLanguage2017] for the unit. The
...
As with the special language/mood used when designing requirements (@subsec-requirements),
...
A class diagram [@UnifiedModelingLanguage2017], containing a collection of blocks and their
...
are truncated to an empty class (as in the `Card` class in @se-cd-fig-agg). One or more optional
...
1. Interface: Indicating the class is an interface. This is usually used to simplify the diagram
...
allow in a class diagram are the aggregation (@se-cd-fig-agg) and the realization (@se-cd-fig-real).
The aggregation connection describes the relationship when one class uses (aggregates) the connected
class. In @se-ex-class, a class diagram for a Go Fish player, we see the `Player` class using
(aggregating) the `Hand` class. In @se-cd-fig-real we see the realization connection, which
describes the relationship when a class implements an interface. An example can be seen in
@se-ex-class with the `Hand` and `Book`[^book] classes realizing the `CardCollection` interface.
...
A state machine [@UnifiedModelingLanguage2017] diagram describes the collection of states and
