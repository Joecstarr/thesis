...
person who has never played "Go Fish," you might be confused by how to ask for a
card. Can the active player ask for a $10$, or should they ask specifically for
a $10\heartsuit$? Fixing this ambiguity in @se-fig-bad_req can by done by making
...
@se-fig-bad_req_fixed is often redundant. Most pieces of software in a rigorous
...
diagram [@UnifiedModelingLanguage2017]. A use case diagram shows the connections
...
case tells) that use case. In @se-ex-usecase we have a connection between use
...
behavior, in @se-ex-usecase we see the `matching` use case included in both the
`fishing` and `drawing` use cases.
