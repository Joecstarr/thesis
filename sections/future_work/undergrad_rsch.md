...
The accessibility of knot theory was discussed in @sec-intro-intuit_knot_theory. This section
...
(@sec-product-management) and developed a software engineering life cycle (@sec-life-cycle) for use
...
Fisher and Frey [@fisherBetterLearningStructured2013].
...
the Mathematical Theory of Knots by Adams [@adamsKnotBookElementary2004]. When the student has
...
knots[^stick] in a program such as Blender or OpenSCAD @sec-proj-sticks, the creation of knot
mosaics @sec-proj-mosaic, or quilting Celtic knots @sec-proj-quilt. The goal of such an activity is
...
[@jablanLinKnotKnotTheory2007] or accessible research papers such as Burton
[@burtonNext350Million2020]. We should encourage freedom in these readings, allowing students to
...
@sec-proj-notations, or the computation of a well understood invariant, as in @sec-projs-invariants.
...
One key issue that must be addressed in a tabulation research program is computational needs.
...
the grafting seen in @sec-rlitt-generation, each computation requires such little computational
...
tangles and the space required to store them in their linearized form (@sec-arborescent-linear).
...
notations for tangles as in @math-listoftangles making retrieval of a specific tangle difficult.
