...
modify a weighted planar tree and form the basis for the classification of the
arborescent knots.
...
subtree lying at odd distance (in the graph theoretic sense) from the vertex shown. Also, apply
$\xi$ ($z$-axis rotation) to all free bonds in the purple subtree that are attached to a vertex at
...
Before we examine more deeply the $F_3^\prime$ move on weighted planar trees, we
will focus on understanding the $F_3^\prime$ move on the band model. The
$F_3^\prime$ move shifts a (or some) crossings across a plumbing square by
shifting a half turn across a plumbing square. When this is carried out, for an
...
{prf:ref}`wpt-moves-def-f3p_move`.
...
as a tangle $T$, we can then carry out the $F_3^\prime$ over this tangle.
Tracking the parts of this operation, we can see the correspondence in
...
{prf:ref}`uc-c-f3-e-cor`. When the $F^\prime_3$ is applied, the child band and
its descendants lay in the interior of the parent band, with the cyclic order of
the child reversed. Observe that descendants odd distance from the parent band,
even from the child, match the cyclic order of the child and are reversed by
$F^\prime_3$. It is easy to see that an even number of crossings shifted will
put the child, and its descendants, back where they started.
...
in orange. The weight of the object vertex has been modified from $-2$ in
...
example, our object weight is even, so applying $F_3^\prime$ the impacted
subtree (purple subtree in {prf:ref}`wpt-moves-fig-f3p_move-indef`) is
unchanged.
...
:width: 500px
 $F_3^\prime$ on the band model realization of the orange vertex.
```

````

In the odd case seen in {prf:ref}`wpt-moves-fig-example_f3-odd`, the object
weight is odd, applying $F_3^\prime$ the cyclic order of vertices, indicated in
red, of the impacted subtree (purple subtree in
{prf:ref}`wpt-moves-fig-f3p_move-indef`) at an odd distance are reversed.
...
:width: 500px
$F_3$ on a weighted planar tree with odd weight clockwise
```
...
Our second move, $F_2$, is a special application of the $F_3^\prime$ move. We
apply $F_3^\prime$ to every child subtree of an object vertex by moving a $1$
weight around a full cycle of the children. The result can be seen in
{prf:ref}`wpt-moves-fig-example_f2`. If a vertex has no weights, the zero weight
is split into a $+1$ and $-1$, one of which shifts around the vertex. When the
cycle is completed, the $+1$ and $-1$ cancel, returning to $0$.

```{figure} ./media/bands/moves/f2/watt_rooted.svg
:label: wpt-moves-fig-example_f2
:width: 500px
$F_2$ on a weighted planar tree
```
...
The third of the $F$ moves is the $F_1$ move, again a special application of the
$F_3^\prime$ move. To realize $F_1$ as a collection of $F_3^\prime$ moves, we
apply both $F_{2e}$ and $F_{2o}$ to the tree, seen in
{prf:ref}`wpt-moves-fig-example_f2`. Observe that the combination of $F_{2e}$
and $F_{2o}$ modifies the free bond by $\xi\eta=\zeta$.
