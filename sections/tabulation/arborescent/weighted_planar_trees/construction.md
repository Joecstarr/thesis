...
half-twists to our plumbing bands (@wpt-construc-fig-6 and
@wpt-construc-fig-25). When forming the half twists, we have two options for
...
@wpt-construc-fig-10. Finally, turning @wpt-construc-fig-10 into a knot is as
...
plumbing from creating "cycles" of bands. That is a chain of plumbing beginning
...
[@bonahonNewGeometricSplittings2016] for the plumbing band construction of
...
tree, a **rooted plane tree**, for our encoding.
...
In a rooted plane tree $\Gamma$, at each vertex $v$, the children of $v$ have an
ordering inherited from the total order of $\Gamma$, we call this ordering of
the children the **cyclic order** of the children. The cyclic order gives us two
...
(seen in @wpt-construc-fig-sticks). We call a stick with 0 free bonds closed, 1
free bond half-open, and 2 free bonds open. Additionally, we call a stick where
each vertex has a single weight a **proper stick**, and we call a vertex on the
end of a stick an **end vertex**.
...
vertex with a single weight $w_0$ we call it an **integral tangle tree**.
...
Finally, we define the **Tree Crossing Number (TCN)** of a weighted planar
