...
\usepackage{qrcode}
\usepackage[left=.5in,right=3in,top=1in,bottom=1in]{geometry}
...
\backgroundsetup{
   ...
   contents={%
   \qrcode{None}
   \tikzset{normal lines/.style={color=black, very thin}}
   \begin{tikzpicture}
       \foreach \y in {1,2,...,40}
       \draw[style=normal lines](0 in, \y*.5 in) -- ( 2.5 in, \y*.5 in);
   \end{tikzpicture}\newline
   }
   }
\usepackage{lineno}
